...
DEFAULT_PRODUCTS = {
    "p1": {"name": "Myntra 100 Off on 199", "price": 30, "file": str(DATA_DIR / "myntra100.txt")},
    "p2": {"name": "Myntra 150 Off on 499", "price": 50, "file": str(DATA_DIR / "myntra150.txt")},
    "p3": {"name": "Myntra 100 off on 399", "price": 25, "file": str(DATA_DIR / "myntra100fwd.txt")},
    "p4": {
        "name": "Myntra Combo(100+100)",
        "price": 55,
        "combo_files": [str(DATA_DIR / "myntra100.txt"), str(DATA_DIR / "myntra100fwd.txt")],
    },
    "p5": {
...
async def paid_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if order.get("payment_mode") == "razorpay_payment_link":
        ...
        await query.message.reply_text("Payment not received yet on Razorpay. Please complete payment first, then tap I Paid again.")
        return
...
async def process_order(chat_id: int, context: ContextTypes.DEFAULT_TYPE, qty: int) -> None:
    await context.bot.send_message(chat_id, "⏳ Creating your order...")
...
def admin_products_keyboard() -> InlineKeyboardMarkup:
    ...
    for pid, product in products.items():
        buttons.append([
            InlineKeyboardButton(f"💰 {pid} | ₹{product['price']} | {product['name'][:18]}", callback_data=f"admin_setprice_{pid}"),
