...
# ENV / CONFIG
# =========================
BOT_TOKEN = os.getenv("BOT_TOKEN_TEST") or os.getenv("BOT_TOKEN")
if not BOT_TOKEN:
    raise RuntimeError("BOT_TOKEN_TEST or BOT_TOKEN env var is required")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.message:
        ...
        await update.message.reply_text("Welcome! Choose option:", reply_markup=main_menu_reply_keyboard())
        await update.message.reply_text("Open any section from below:", reply_markup=main_menu_inline_keyboard())
...
async def select_quantity(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "qty_custom":
        ...
        await query.message.reply_text("Enter custom quantity:", reply_markup=back_to_menu_inline_keyboard())
        return
...
async def process_order(chat_id: int, context: ContextTypes.DEFAULT_TYPE, qty: int) -> None:
        ...
        try:
            ...
            await context.bot.send_message(
                ...
                (
                    ...
                    f"✅ After successful payment, codes will be delivered automatically."
                ),
            ...
            await context.bot.send_message(
                ...
                (
                    ...
                    f"Order ID: {order_id}\n"
                    f"You can send manual payment screenshot for backup approval."
                ),
                reply_markup=back_to_menu_inline_keyboard(),
            )
    ...
    summary = (
        ...
        f"Order ID: {order_id}\n\n"
        f"💳 UPI ID: `{upi}`\n"
        f"Scan the QR below or copy this UPI ID and pay.\n"
        f"After payment, send screenshot here."
    )
    await context.bot.send_message(chat_id, summary, reply_markup=back_to_menu_inline_keyboard(), parse_mode="Markdown")
        ...
        with open(qr, "rb") as photo:
            await context.bot.send_photo(
                ...
                caption=(
                    ...
                    f"💳 UPI ID: `{upi}`\n"
                    f"Copy UPI or scan and pay.\n"
                    f"After payment, send screenshot here."
                ),
                parse_mode="Markdown",
            )
    except Exception:
        await context.bot.send_message(chat_id, f"QR not found\n\nUPI ID: `{upi}`", parse_mode="Markdown")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not order:
        await update.message.reply_text("Please create an order first", reply_markup=main_menu_reply_keyboard())
        return

    if order.get("status") == "approved":
        await update.message.reply_text("This order is already paid and delivered.", reply_markup=main_menu_reply_keyboard())
        return
    ...
    update_order(order_id, status="manual_review_pending", admin_messages=admin_messages)
    await update.message.reply_text(f"Waiting for manual approval\nOrder ID: {order_id}", reply_markup=back_to_menu_inline_keyboard())
...
async def deliver_order_automatically(order_id: str, source: str, payment_data: Optional[Dict[str, Any]] = None) -> bool:
    ...
    if product_id not in products:
        ...
        if BOT_APP:
            await BOT_APP.bot.send_message(order["user_id"], "Sorry, product is invalid now.", reply_markup=main_menu_reply_keyboard())
        return False
...
async def admin_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "reject":
        ...
        order = find_order(order_id) or order
        await clear_admin_buttons(context, order)
        await context.bot.send_message(order["user_id"], "❌ Payment rejected", reply_markup=main_menu_reply_keyboard())
        for admin in ALL_ADMINS:
            try:
                await context.bot.send_message(admin, f"❌ Rejected by {query.from_user.first_name}\n\n🧾 Order ID: {order_id}")
            except Exception:
                pass
...
async def admin_history(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for item in history[-25:][::-1]:
        ...
        lines.append(
            ...
            f"📌 Status: {item.get('status')}{delivered_info}\n"
...
def admin_panel_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("📢 Broadcast", callback_data="admin_broadcast"), InlineKeyboardButton("🔄 Refresh", callback_data="admin_home")],
        ]
    )
...
async def send_recent_orders(chat_id: int, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for item in history[-15:][::-1]:
        lines.append(
            f"{item.get('order_id', '-')} | {item.get('product_name')} | Qty {item.get('qty')} | ₹{item.get('total')} | {item.get('status')}"
        )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    navigation_texts = {"🛒 buy", "buy", "📦 my orders", "my orders", "❓ help", "help", "/start", "/help", "/admin"}
...
class WebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if self.path.startswith("/paid"):
            ...
            self.end_headers()
            self.wfile.write(
                b"<html><body><h2>Payment received.</h2><p>You can return to Telegram. Codes are delivered automatically after confirmation.</p></body></html>"
            )
            return
    ...
    def do_POST(self) -> None:
        ...
        if BOT_LOOP:
            future = asyncio.run_coroutine_threadsafe(
                deliver_order_automatically(order_id, "Razorpay webhook", payment_data=payload),
                BOT_LOOP,
            )
            try:
                future.result(timeout=60)
            except Exception as e:
                logger.exception("Webhook delivery task failed: %s", e)
                self._send_json(500, {"ok": False, "error": "delivery_failed"})
                return
...
async def post_init(app: Application) -> None:
    ...
    await app.bot.set_my_commands([
        ...
        BotCommand("history", "My order history"),
