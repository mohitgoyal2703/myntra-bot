...
DEFAULT_PRODUCTS = {
     "p1": {"name": "Myntra 100 Off on 199", "price": 30, "file": str(DATA_DIR / "myntra100on199.txt"), "reserve": 2},
    "p2": {"name": "Myntra 150 Off on 649", "price": 30, "file": str(DATA_DIR / "myntra150on649.txt"), "reserve": 2},
    
     "p3": {
        "name": "Myntra Combo(100+150)",
        "price": 60,
        "combo_files": [str(DATA_DIR / "myntra100on199.txt"), str(DATA_DIR / "myntra150on649.txt")],
        "reserve": 2,
    },
     
    "p4": {"name": "Myntra 100 off on 499", "price": 25, "file": str(DATA_DIR / "myntra100on499.txt"), "reserve": 2},
    "p5": {"name": "Myntra FWD 100 off on 399", "price": 25, "file": str(DATA_DIR / "myntra100on399.txt"), "reserve": 2},
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if update.message.from_user.id not in MAIN_ADMINS or not context.user_data.get("awaiting_data_file_upload"):
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        "❓ Need Help?\n\n📩 Contact: @myntracodes\n\nFor Razorpay, pay first and then tap I Paid to receive codes.",
...
def admin_data_files_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("📋 List Files", callback_data="admin_datafiles_list")],
            [InlineKeyboardButton("📥 Upload / Overwrite", callback_data="admin_datafiles_upload")],
...
async def send_data_files_list(chat_id: int, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await context.bot.send_message(chat_id, "\n".join(lines), reply_markup=build_data_file_export_keyboard())
